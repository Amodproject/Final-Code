...
def createbundles(product_recs_df):
    pivot_df = pd.pivot_table(df2,index = 'Customer ID',columns = 'Product Name',values = 'Sub-Category',aggfunc = 'count')
    pivot_df.reset_index(inplace=True)
    pivot_df = pivot_df.fillna(0)
    pivot_df = pivot_df.drop('Customer ID', axis=1)
    ...
    for i in cos_score_df.index:
        product_recs.append(cos_score_df[cos_score_df.index!=i][i].sort_values(ascending = False)[0:2].index)
    ...
    product_recs_df=product_recs_df.set_axis([ 'Recommendation 1', 'Recommendation 2','Recommendation List'], axis=1)
    st.table(product_recs_df)
...
def create_barplot_cw(df4,rev_count42):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    rev_count2=df4.groupby(['Ship Mode']).sum()['Sales']
    # rev_count42
